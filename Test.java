...
public class Test {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Cliente cliente1 = new Cliente(contacto1, "Alaniz", "Nahuel", 'm', 12345678);
			...
			comercio1.agregarCarrito(fecha1, LocalTime.now(), cliente1);
			...
			carrito1.sacarDelCarrito(comercio1.traerArticulo(2), 5); // ACA PROBAMOS SACAR UN ARTICULO
			
			System.out.println("IMPRIMIMOS LISTA DE CARRITO E ITEMS");
			comercio1.mostrarListaCarrito();
			carrito1.mostrarListaItemCarrito();
		...
		try {
			...
			Cliente cliente1 = new Cliente(contacto1, "Alaniz", "Nahuel",'m', 0); // PROBAMOS UN DNI INVALIDO

		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
		
		try {//LANZA EXCEPCION SI EL SEXO INGRESADO ES INCORRECTO
			Ubicacion ubicacion1 = new Ubicacion(-34.9214, -57.9544);
			Contacto contacto1 = new Contacto("nahuel@outlook.es", "1154786546", ubicacion1);
			Cliente cliente1 = new Cliente(contacto1, "Nahuel", "Alaniz", 'r', 43627128);
		}catch (Exception e) {
			System.out.println(e.getMessage());
		}
